...
var myVue = new Vue({
	...
	computed: {
		...
		filteredMembers: function () {
			var arr = []; // this [arr] will contain only the user's selection e.g. ["R", "I", "WY", "AL", "NC"]
			// adding R, D or I depending on the Checked boxes...
			var rdi = ["R", "D", "I"];
			console.log(document);
			for (var i = 0; i < rdi.length; i++) {
				if (document.querySelectorAll('input[value='+ rdi[i] +']:checked').length == 1) {
					arr.push(rdi[i]);
				}
			}
			console.log(arr);
			...
			console.log(document.querySelectorAll('#selectDropDown option:checked'));
			// adding the states selected in the DropDown Menu...		
			
			
//			if ((document.querySelectorAll('#selectDropDown option:checked') == 0) 
//					|| (document.querySelectorAll('#selectDropDown option:checked')[0].value === 'all')
//				 ) {
//				arr = arr.concat(this.stateList);
//			} else {
//				arr = arr.concat(Array.from(document.querySelectorAll('#selectDropDown option:checked')).map(abc => abc.value));
//			}
			
			
			//  at this point [arr] this should contain something like: ["R", "I", "WY", "AL", "NC"]
			// Now it's time to compare [arr] with [this.members] and pick only the members matching the elements in [arr]
			var arrFiltered = [];
			for (var i = 0; i < this.members.length; i++) {
				if ((arr.includes(this.members[i].party)) && (arr.includes(this.members[i].state))) {
					arrFiltered.push(this.members[i]);
				}
			}
